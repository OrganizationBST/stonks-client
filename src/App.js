...
class App extends Component {
    ...
    if (!this.state.dates.length) {
      ...
      while (currentDate <= stopDate) {
        if (currentDate.day() !== 6 && currentDate.day() !== 0) {
          dateArray.push( moment(currentDate).format('M/D') )
        }
  ...
  getChartData = (ticker) => {
    if (ticker === '') {
      this.setState({ error: 'Please enter a symbol.'})
    }
    else {
      this.setState({ error: ''});
      axios.post('/api/predict', null, { params: { ticker }})
        .then(res => {
          ...
          const color = actual[actual.length - 1] > actual[0] ? '#34c759' : '#ff3a2f';
          const gradient = actual[actual.length - 1] > actual[0] ? this.state.gradients[0] : this.state.gradients[1];
